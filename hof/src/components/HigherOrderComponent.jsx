...
class HigherOrderComponent extends Component {
    ...
    designerType = () => {
        const data = this.state.userData;
        const design = data.filter((value) => value.user_type === 'Designer')
            .map((value) =>
                <div key={value.id}>
                    <li className="list-elements">
    ...
    letterJ = () => {
        const data = this.state.userData;
        const startj = data.filter((value) => value.name[0] === "J")
            .map((value) =>
                <div key={value.id}>
                    <li className="list-elements">
                        <span>Id: {value.id}</span>
    ...
    totalYears = () => {
        const data = this.state.userData;
        const tally = data.filter((value) => value.user_type === "Designer")
